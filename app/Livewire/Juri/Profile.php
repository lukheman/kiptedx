...
class Profile extends Component
{
    ...
    {
        $this->juri = Auth::guard('juri')->user();
    }
    ...
    public function updateFotoProfil()
    {
        if ($this->cropped_foto) {
            ...
            $fileName = 'foto-juri/'.uniqid().'.'.$extension;
            Storage::disk('public')->put($fileName, $image_base64);

            if ($this->juri->foto_profil) {
                Storage::disk('public')->delete($this->juri->foto_profil);
            }

            $this->juri->foto_profil = $fileName;
            $this->juri->save();
    ...
    public function updatePassword()
    {
        ...
        if (! Hash::check($this->current_password, $this->juri->password)) {
            $this->addError('current_password', 'Password saat ini salah.');

            return;
        }

        $this->juri->password = Hash::make($this->password);
        $this->juri->save();
    ...
    {
        return view('livewire.juri.profile');
    }
}
