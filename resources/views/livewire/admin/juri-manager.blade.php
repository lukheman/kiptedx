<div>
    {{-- Page Header --}}
    <x-page-header title="Manajemen Juri" subtitle="Kelola data juri">
        <x-slot:actions>
    ...
    <div class="modern-card">
        ...
        <div class="d-flex justify-content-between align-items-center mb-4">
            <h5 class="mb-0" style="color: var(--text-primary); font-weight: 600;">Daftar Juri</h5>
            ...
            <table class="table table-modern">
                ...
                <tbody>
                    @forelse ($juris as $juri)
                        <tr wire:key="juri-{{ $juri->id }}">
                            <td class="text-center">
                                @if ($juri->hasAvatar())
                                    <img src="{{ $juri->avatarUrl() }}" alt="Foto" style="width: 40px; height: 40px; border-radius: 50%; object-fit: cover; border: 2px solid var(--border-color);">
                                @else
                            ...
                            <td style="color: var(--text-secondary); font-weight: 600;">{{ $juri->nim }}</td>
                            <td>
                                <div class="fw-semibold" style="color: var(--text-primary);">{{ $juri->nama }}</div>
                            </td>
                            <td>
                                <div class="d-flex gap-1">
                                    <button class="action-btn action-btn-edit" wire:click="openEditModal({{ $juri->id }})" title="Edit Juri">
                                        <i class="fas fa-edit"></i>
                                    </button>
                                    <button class="action-btn action-btn-delete" wire:click="confirmDelete({{ $juri->id }})" title="Hapus Juri">
            ...
            <div class="modal-content-custom" wire:click.stop>
                <div class="modal-header-custom">
                    <h5 class="modal-title-custom">
                        {{ $editingId ? 'Edit Juri' : 'Tambah Juri' }}
                    </h5>
                ...
                <form wire:submit="save">
                    ...
                    <div class="d-flex justify-content-end gap-2 mt-4">
                        ...
                        <x-button type="submit" variant="primary">
                            {{ $editingId ? 'Simpan Perubahan' : 'Simpan Juri' }}
                        </x-button>
                    </div>
                </form>
            </div>
    ...
    <x-confirm-modal
        ...
        message="Apakah Anda yakin ingin menghapus data juri ini? Tindakan ini tidak dapat dibatalkan."
        on-confirm="deleteJuri"
